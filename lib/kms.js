  ...
  return co(function * () {
    ...
    try {
      kmsResponse = yield request(aws4.sign({
        ...
        body: JSON.stringify({ CiphertextBlob: new Buffer(text).toString('base64') })
      }))
    }
    let result = JSON.parse(kmsResponse.data.toString())
    if (result.err.message && result.err.message.indexOf('The ciphertext references a key that either does not exist or you do not have access to') !== -1) {
